<div>
    <div class="mb-6 flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        ...
        <a href="{{ route('purchase-returns.create', ['receipt' => $receipt->id]) }}" class="inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm transition hover:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700">
            Create Return
        </a>
    </div>
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800 lg:col-span-1">
            <h2 class="mb-4 text-lg font-semibold text-gray-900 dark:text-white">Receipt Summary</h2>
            <dl class="space-y-3 text-sm">
                <div class="flex justify-between gap-3"><dt class="text-gray-500 dark:text-gray-400">Supplier</dt><dd class="text-right text-gray-900 dark:text-white">{{ $receipt->supplier?->name }}</dd></div>
                <div class="flex justify-between gap-3"><dt class="text-gray-500 dark:text-gray-400">Location</dt><dd class="text-right text-gray-900 dark:text-white">{{ $receipt->stockLocation?->name }}</dd></div>
                <div class="flex justify-between gap-3"><dt class="text-gray-500 dark:text-gray-400">Status</dt><dd class="text-right text-green-600 dark:text-green-400">{{ $receipt->status->label() }}</dd></div>
                <div class="flex justify-between gap-3"><dt class="text-gray-500 dark:text-gray-400">Total</dt><dd class="text-right font-semibold text-gray-900 dark:text-white">{{ $currencyFormatter->formatValue($receipt->subtotal_amount, 2) }}</dd></div>
                <div class="flex justify-between gap-3"><dt class="text-gray-500 dark:text-gray-400">Created By</dt><dd class="text-right text-gray-900 dark:text-white">{{ $receipt->creator?->name ?? 'System' }}</dd></div>
            </dl>
            @if($receipt->notes)
                <div class="mt-6 border-t border-gray-200 pt-4 text-sm text-gray-600 dark:border-gray-700 dark:text-gray-300">{{ $receipt->notes }}</div>
            @endif
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-900">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Inventory Item</th>
                        ...
                        @foreach($receipt->items as $item)
                            <tr>
                                ...
                                <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">
                                    ...
                                    @if($item->expiry_date)
                                        <div>Expiry {{ $item->expiry_date->format('M d, Y') }}</div>
                                    @endif
                                </td>
