<x-layouts.app title="Edit Product Category">
    <div class="mb-6">
        <a href="{{ route('product-categories.index') }}" class="text-blue-600 hover:text-blue-900 dark:hover:text-blue-400 mb-2 inline-block">
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Update product category details.</p>
    </div>
        ...
        <form action="{{ route('product-categories.update', $productCategory) }}" method="POST">
            ...
            <div class="mb-5">
                ...
                <textarea name="description" id="description" rows="3"
                    class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md dark:bg-gray-700 dark:text-white text-sm focus:ring-blue-500 focus:border-blue-500">{{ old('description', $productCategory->description) }}</textarea>
            ...
            <div class="mb-6">
                <label class="flex items-center cursor-pointer">
                    ...
                    <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">Active (can be selected when adding new products)</span>
                </label>
            </div>

            <div class="pt-4 border-t border-gray-200 dark:border-gray-700 flex justify-end">
                <a href="{{ route('product-categories.index') }}" class="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 mr-3 transition">Cancel</a>
                <button type="submit" class="px-6 py-2 bg-blue-600 text-white rounded-md shadow-sm text-sm font-medium hover:bg-blue-700 transition">Update Category</button>
            </div>
        </form>
    </div>
</x-layouts.app>
