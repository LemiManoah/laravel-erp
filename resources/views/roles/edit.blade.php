<x-layouts.app title="Edit Role">
    <div class="mb-6">
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Update role name and modify granted permissions.</p>
    </div>
        ...
        <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg p-6 border border-gray-200 dark:border-gray-700 mb-6">
            <div class="mb-8 max-w-md">
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $role->name) }}" required
                    ...
                    class="{{ $role->name === 'Admin' ? 'bg-gray-100 dark:bg-gray-900 text-gray-500 cursor-not-allowed' : 'dark:bg-gray-700 dark:text-white' }} w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md text-sm focus:ring-blue-500 focus:border-blue-500">
                @if($role->name === 'Admin')
                    <p class="mt-1 text-xs text-red-500">The Admin role name cannot be modified.</p>
                @endif
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6" x-data="permissionsManager({{ \Illuminate\Support\Js::from(old('permissions', $rolePermissions)) }}, {{ \Illuminate\Support\Js::from($permissions->keys()) }})">
            ...
            <div class="mt-8 pt-5 border-t border-gray-200 dark:border-gray-700 flex justify-end">
                ...
                <button type="submit" class="px-6 py-2 bg-blue-600 text-white rounded-md shadow-sm text-sm font-medium hover:bg-blue-700 transition">Update Role</button>
            </div>
        </div>
    </form>
