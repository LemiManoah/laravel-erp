<x-layouts.app title="Edit Expense Category">
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Edit Category: {{ $expenseCategory->name }}</h1>
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Modify category details and availability.</p>
    </div>
        ...
        <form action="{{ route('expense-categories.update', $expenseCategory) }}" method="POST">
            ...
            <div class="mb-5">
                ...
                <textarea name="description" id="description" rows="3"
                    class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md dark:bg-gray-700 dark:text-white text-sm focus:ring-blue-500 focus:border-blue-500">{{ old('description', $expenseCategory->description) }}</textarea>
            ...
            <div class="mb-6">
                ...
                @if(!$expenseCategory->is_active)
                    <p class="mt-2 text-xs text-red-500 dark:text-red-400">This category is currently inactive. It will not appear in dropdowns for new expenses.</p>
                @endif
            </div>
