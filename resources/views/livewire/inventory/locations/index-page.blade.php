<div>
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Stock Locations</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400">Manage warehouses, stores, and storage areas.</p>
        </div>
        <div class="flex gap-2">
            @can('stock-locations.create')
                <a href="{{ route('inventory.stock-locations.create') }}" class="inline-flex w-full items-center justify-center rounded-md border border-transparent bg-blue-600 px-4 py-2 text-sm font-medium text-white shadow-sm transition hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 sm:w-auto">
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-900">
                    <tr>
                        ...
                        <th class="w-1/6 px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Code</th>
                        <th class="w-1/6 px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Type</th>
                        <th class="w-1/12 px-6 py-3 text-center text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Default</th>
                        ...
                        <tr wire:key="location-row-{{ $location->id }}">
                            ...
                            <td class="whitespace-nowrap px-6 py-4">
                                <span @class([
                                    ...
                                    'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' => $location->is_active,
                                    'bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-400' => ! $location->is_active,
                                ])>
                            ...
                            <td class="whitespace-nowrap px-6 py-4 text-right text-sm font-medium">
                                <div class="flex justify-end gap-2">
                                    @can('stock-locations.update')
                                        <a href="{{ route('inventory.stock-locations.edit', $location) }}" class="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-300">
                                            Edit
                                        </a>
                                        <button type="button" wire:click="confirmDelete({{ $location->id }})" class="ml-2 text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300">
                                            Delete
                                        </button>
                                    @endcan
                                </div>
    ...
    @if($confirmingDeletion)
        <div class="relative z-50" aria-labelledby="location-delete-modal-title" role="dialog" aria-modal="true">
                    ...
                    <div class="relative transform overflow-hidden rounded-lg border border-gray-200 bg-white text-left shadow-xl transition-all dark:border-gray-700 dark:bg-gray-800 sm:my-8 sm:w-full sm:max-w-lg">
                        <div class="bg-white px-4 pb-4 pt-5 dark:bg-gray-800 sm:p-6 sm:pb-4">
                            <div class="text-center sm:text-left">
                                <h3 class="text-lg font-semibold leading-6 text-gray-900 dark:text-white" id="location-delete-modal-title">
                                    Delete Confirmation
                                </h3>
                                <div class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                                    <p>Are you sure you want to delete <strong class="break-words">{{ $deletingLocationName }}</strong>? This action cannot be undone.</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="border-t border-gray-200 bg-gray-50 px-4 py-3 dark:border-gray-700 dark:bg-gray-900 sm:flex sm:flex-row-reverse sm:px-6">
                            <button type="button" wire:click="deleteLocation" class="inline-flex w-full justify-center rounded-md bg-red-600 px-3 py-2 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-red-500 sm:ml-3 sm:w-auto">
                                Delete
                            </button>
