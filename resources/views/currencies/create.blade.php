<x-layouts.app title="Create Currency">
    <div class="mb-6">
        <a href="{{ route('currencies.index') }}" class="text-blue-600 hover:text-blue-900 dark:hover:text-blue-400 mb-2 inline-block">
            Back to Currencies
        </a>
        <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Create Currency</h1>
    </div>

    <div class="max-w-3xl">
        <form action="{{ route('currencies.store') }}" method="POST" class="bg-white dark:bg-gray-800 shadow-sm rounded-lg border border-gray-200 dark:border-gray-700 p-6 space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="code" id="code" value="{{ old('code', 'UGX') }}" maxlength="3" required
                ...
                <div>
                    ...
                    <input type="text" name="symbol" id="symbol" value="{{ old('symbol', 'UGX') }}" required
                        class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md dark:bg-gray-700 dark:text-white text-sm">
                    @error('symbol')
                        <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" name="decimal_places" id="decimal_places" min="0" max="4" value="{{ old('decimal_places', 0) }}" required
                        class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md dark:bg-gray-700 dark:text-white text-sm">
                ...
                <div>
                    ...
                    <input type="number" name="exchange_rate" id="exchange_rate" min="0" step="0.000001" value="{{ old('exchange_rate', 1.000000) }}" required
                        class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md dark:bg-gray-700 dark:text-white text-sm">
                    @error('exchange_rate')
                        <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" name="sort_order" id="sort_order" min="0" value="{{ old('sort_order', 0) }}" required
                        class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md dark:bg-gray-700 dark:text-white text-sm">
                    @error('sort_order')
                        <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <label class="flex items-center gap-3">
                    ...
                    <input type="checkbox" name="is_default" value="1" @checked(old('is_default', false))
                        class="rounded border-gray-300 text-blue-600 shadow-sm focus:ring-blue-500">
                    <span class="text-sm text-gray-700 dark:text-gray-300">Make default currency</span>
                </label>
            </div>
